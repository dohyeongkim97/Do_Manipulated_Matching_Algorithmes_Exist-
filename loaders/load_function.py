...
class RiotAPIClient:
    ...
    def get_summoners_by_division(self, tier, division, queue='RANKED_SOLO_5x5'):
        url = f'https://kr.api.riotgames.com/lol/league-exp/v4/entries/{queue}/{tier}/{division}'
        params = {"page": 1}
        res = requests.get(url, headers=self.headers, params=params, timeout=7)
        time.sleep(1.0)
        if res.status_code != 200:
            print(f"[{tier}-{division}] {res.status_code} | {res.text}")
            return pd.DataFrame()
        return pd.DataFrame(res.json())

    def get_matchlist(self, puuid, count=100):
        url = f'https://asia.api.riotgames.com/lol/match/v5/matches/by-puuid/{puuid}/ids'
        params = {"start": 0, "count": count}
        res = requests.get(url, headers=self.headers, params=params, timeout=7)
        time.sleep(1.0)
